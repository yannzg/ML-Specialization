...
# Try to import ipywidgets, but make it optional
try:
    from ipywidgets import interact
except ImportError:
    interact = None  # Will cause errors if used, but allows file to be imported
...
# Try to use custom style, but don't fail if it doesn't exist
try:
    import os
    _style_path = os.path.join(os.path.dirname(__file__), 'deeplearning.mplstyle')
    plt.style.use(_style_path)
except OSError:
    pass  # Style file not found, use default matplotlib style
n_bin = 5
